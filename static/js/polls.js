...
var AllPolls = React.createClass({
    ...
    render: function(){
        ...
        // if a message was returned in the json result (the poll wasn't found) /api/poll/<poll_name>
        if(!this.state.polls.message){

            return (
                <LivePreviewProps polls={this.state.polls} loadPollsFromServer={this.loadPollsFromServer} header={this.state.header} classContext={this.state.classContext} /> 
            );
        } else {
            return (
                <div style={Align}>
